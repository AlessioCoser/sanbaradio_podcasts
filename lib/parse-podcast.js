const parsePodcast = function (payload){
  ...
  var getTitleWithoutCategory = function () {
    var searchCategory = new RegExp(escape(getCategory()),"i");
    var title = getTitle()
    var match = title.match(searchCategory)

    if (match) {
      var category = match[0]

      return title.split(category + ' - ').join('')
      .split(category + ' ').join('')
      .split(category).join('')
    } else {
      return title
  ...
  var getEpisode = function () {
    ...
    if (subtitle == undefined || subtitle == null || subtitle == '') {
      return getTitleWithoutCategory()
    }
